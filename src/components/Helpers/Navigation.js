          ...
          .then(jsonresponse => {
            if (jsonresponse.data.lenght === 0) {
              ...
              content = jsonresponse.data.map((lista, index) => {
                return (
                  <a
                    className={` ${lista.uuid == this.props.infocuenta['uuid'] ? 'active' : ''}`}
                    onClick={e => {
                      lista.code == null ? '' : this.select(e, lista.uuid);
                    }}
                    key={index}
                    href="#"
                  >
                    {lista.code == null ? 'Cuenta por aprobar' : lista.code}
                  </a>
                );
              });
              this.setState({
    ...
    return (
      <div className="navigation">
        <div className={`dropdown ${this.props.scope === 'user' ? '' : 'display-none '}`}>
          <button className="dropbtn" onClick={this.showcounts}>
            <i className="md-icon">loop</i> <span>Cuenta </span>
          </button>
          <div
            className={`dropdown-content ${
              this.state.counts === true ? ' dropdown-content-show' : ''
            }`}
          >
            {this.state.codeacount}
            <a className="add-acount" href="#newacount" onClick={this.show_overlay_new_acount}>
              <i className="md-icon">playlist_add</i> Agregar nueva cuenta.
            </a>
          </div>
        </div>
        ...
        <div
          id="newa_count"
          className={`overlay ${this.state.overlay_new_acount === true ? 'overlay_visible' : ''}`}
        >
          <div className="popup popup_new_acount">
            <div className="popup-title">
              <h4>Agregar Nueva Cuenta</h4> <hr />{' '}
              <a className="close" onClick={this.show_overlay_new_acount}>
                x
              </a>
            </div>
            <div className="popup-body">
              <div className="popup-acountnew">
                <div className="form-group">
                  <p>
                    Si deseas crear otra cuenta de inversion con tus mismos datos personales, debes
                ...
                <div className="form-group">
                  <NumberFormat
                    ...
                    className={`form-control ${
                      this.state.initial_amount_validate === 'error' ? 'error' : ''
                    }`}
                ...
                <div className="form-group">
                  <select
                    className={`form-control ${
                      this.state.origin_of_funds_validate === 'error' ? 'error' : ''
                    }`}
                ...
                <div className="form-group hcht40">
                  <input
                    ...
                    className={`custom-file-input ${
                      this.state.voucher_validate === 'error' ? 'error' : ''
                    }`}
                    id="voucher"
                    name="voucher"
                ...
                <div className="form-group">
                  <select
                    className={`form-control ${
                      this.state.bank_deposit_validate === 'error' ? 'error' : ''
                    }`}
                    id="bank_deposit"
                    name="bank_deposit"
                    onChange={this.onChange}
                ...
                <div className="form-group">
                  <select
                    className={`form-control ${
                      this.state.transfer_format_validate === 'error' ? 'error' : ''
                    }`}
                    id="transfer_format"
                  ...
                  >
                    ...
                    <option value="Cheque de Caja(de Gerencia)">
                      {' '}
                      Cheque de Caja(de Gerencia){' '}
                    </option>
                    ...
                    <li>
                      <div>
                        <span>
                          ...
                          que conlleva el sistema de referido de UEES.
                        </span>
                      </div>
                    </li>
                  </ul>
                ...
                <div className="form-group section-btn">
                  <a
                    className={`blue btn ${this.state.acept_terms === true ? '' : 'display-none'}`}
                    href="#"
                    onClick={this.validate_new_acount}
                  >
                    {' '}
                    Aceptar
                  </a>
        ...
        <div
          id="add_new_acount_confirm"
          className={`overlay ${
            this.state.overlay_new_acount_confirm === true ? 'overlay_visible' : ''
          }`}
        >
          <div className="popup">
            <div className="popup-title">
            ...
            <div className="popup-body">
              ...
              <div className="popup-acountnew">
                <div className="form-group section-btn">
                  <a className="close blue btn" href="" onClick={this.overlay_new_acount_confirm}>
                    CANCELAR
                  </a>{' '}
                  <a
                    className="blue btn"
