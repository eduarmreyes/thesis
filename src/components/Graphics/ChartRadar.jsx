...
class ChartRadar extends Component {
  ...
  customTooltips(tooltip, _this) {
    ...
    if (!tooltipEl) {
      ...
      _this.chartradar.canvas.parentNode.appendChild(tooltipEl)
    }
  ...
  componentDidMount() {
    ...
    const chartradar = new Chart(ctx, options)

    // console.log("Children", "Exist update method?", "update" in chart)
     console.log(chartradar)

    this.setState({ chartradar })

    if (typeof this.props.callbackradar === "function") {
      this.props.callbackradar(chartradar)
    }
  }
