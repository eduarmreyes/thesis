...
class Pasocinco extends Component{
	constructor(props){
		super(props)
		this.state = {
			stado:'',
  ...
  submitFiles(data_files, bearer) {
  	axios({
  		method: 'put',
  		url: this.props.baseurl + '/v1/assets/users/upload_voucher',
  		data: data_files,
  		headers: { 'Authorization': bearer,'Content-Type': 'application/json' }
  	})
  	.then(jsonresponse => {
  ...
  onChange(e) {
	...
	componentDidMount(){
		const bearer = 'bearer '+ this.props.userToken;
		axios.get(this.props.baseurl + '/v1/user_accounts', {
      headers: { 'Authorization': bearer, 'Content-Type': 'application/json' }
		})
		.then(jsonresponse => {
					...
					<div className="col-5">
						<div className="register--information">
							...
							<ul className="register--steps-number">
								...
								<li className="register-step"><a className="register-step--link active" to="empasocinco">5</a></li>
							</ul>
						</div>
					</div>
								...
								<div className="register--form--wrapper">
									...
									<select
										...
										value={
											...
											(this.state.transfer_format === "electronic") :
											''
										}
										onChange={this.onChange}
										>
										...
										<option value={true}>Wire Transfer</option>
									</select>
								</div>
							</div>
							<div className="register--form-d-flex">
	                ...
	                <input className="custom-file-input" required type="file" name="voucher" id="voucher" />
							...
							<div className="register--form-d-flex">
								<div className="register--form--buttons-right">
									<Link to="/empasocuatro" className="btn btn-back">Atrás</Link>
								</div>
...
export default withRouter(connect(mapStateToProps)(Pasocinco));
