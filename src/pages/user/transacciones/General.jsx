  	...
  	get_transaction_filter(e,page,per_page){
		//e.preventDefault()
     	...
     	var date2 = enddate.format('YYYY-MM-DD');
	      ...
	      axios.request('GUET', {
	        url:this.props.baseurl + '/v1/user_filters/transactions?account_uuid='+this.state.id_cuenta+'&status='+ status +'&start_date='+ date1 +'&end_date='+ date2 +'&page='+ page +'&per_page='+ per_page +'',
	        headers: { 'Authorization': bearer,'Content-Type': 'application/json' }
	      })
	      .then(jsonresponse => { 
	       let content;
			...
			<Backend title={this.state.title} page={this.state.page} menu={this.state.menu}>			
				<div className="page-cliente-empty">
					...
					<ul className="left filter-list">
						...
						<li>
							<a href={null} onClick={this.changuefilter}> <i className="md-icon">find_in_page</i> <span>Filtrar</span> </a>
						</li> 
