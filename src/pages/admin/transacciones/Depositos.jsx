...
class Depositos extends Component{
	...
	componentDidMount(){
  	...
  	get_depositos(page,per_page){
	      ...
	      axios.request('GUET', {
	        url:this.props.baseurl + '/v1/transactions/trans_deposit?page='+ page +'&per_page='+ per_page +'',
	      ...
	      .then(jsonresponse => { 
	       ...
	       content = jsonresponse.data.map((lista, index) => {			     
		        ...
		        <tr key={index} className="no-cursorpointer">
					...
					<td> {lista.amount}</td>
					<td> { lista.trans_gaining === null ? "N/A" : lista.trans_gaining }</td>
  	...
  	get_depositos_filter(e,page,per_page){
  		const bearer = 'bearer '+ this.props.userToken;
     	...
     	var date2 = enddate.format('YYYY-MM-DD');
	      ...
	      axios.request('GUET', {
	        url:this.props.baseurl + '/v1/transactions/trans_deposit?start_date='+ date1 +'&end_date='+ date2 +'&page='+ page +'&per_page='+ per_page +'',
	        headers: { 'Authorization': bearer,'Content-Type': 'application/json' }
	      })
	      .then(jsonresponse => { 
	       let content;
 	...
 	get_csv(){
      ...
      axios.request('GUET', {
        url:this.props.baseurl + '/v1/transactions/trans_deposit?page='+ this.state.pagina +'&per_page='+ 15 +'',
        headers: { 'Authorization': bearer,'Content-Type': 'application/json' }
      })
      .then(jsonresponse => { 
	...
	get_filter(){
		...
		const content_filter_two= <ul className="filter_date">
			...
			<li>
	          ...
	          <li>
	            <a className="btn-blue blue" href={null} onClick={e => {this.get_depositos_filter(e,1,15)}} > Aplicar</a>
	          </li>              
			<li>
