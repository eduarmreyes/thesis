...
class LoginUser extends Component {
  ...
  get_my_account_info() {
    const bearer = 'bearer '+ this.props.userToken;
    axios.get(this.props.baseurl + '/v1/user_accounts?page=1&per_page=100', {
      headers: { 'Authorization': bearer, 'Content-Type': 'application/json' }
    })
    .then(jsonresponse => {
    ...
    const view_login =
      <div className="page-empty-content">
        ...
        <form className="border-radius-bottom border-0top" onSubmit={this.onSubmit}>
          <div className="form-group"><label className="label-gray-color">Usuario</label><input type="text" name="email" className="form-control" required onChange={this.onChange} />
          </div>
          ...
          <div className="form-group form-group-button">
            <div className="form-group-button-description content-aling-center label-gray-color">Crea tu cuenta, <a className="blue-link" href="/singup">Click</a></div>
          </div>
        </form>
      </div>
      this.setState({
